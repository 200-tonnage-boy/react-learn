...
export class Component {
  ...
  forceUpdate() {
    ...
    //添加新生命周期：getDerivedStateFromProps
    if (newVDOM.type.getDerivedStateFromProps) {
      // 该生命周期是静态属性，通过类本身来调用
      let newState = newVDOM.type.getDerivedStateFromProps(
        this.props,
        this.state
      ); //Q:这里的props是最新的吗？
      if (newState) {
        // 该生命周期钩子返回值不为空的话将该对象与state进行合并；
        this.state = { ...this.state, ...newState };
      }
    }
    // 实现getSnapshotBeforeUpdate钩子，该钩子的返回值将作为额外参数传递给componentDidUpdate
    let extraArgs =
      this.getSnapshotBeforeUpdate && this.getSnapshotBeforeUpdate(); // 因为此时还没有创建新DOM对象
